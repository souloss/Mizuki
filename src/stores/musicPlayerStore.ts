...
class MusicPlayerStore {
	...
	private async loadPlaylist(): Promise<void> {
		...
		const meting_api =
			musicPlayerConfig.meting?.api ??
			"https://www.bilibili.uno/api?server=:server&type=:type&id=:id&auth=:auth&r=:r";
		const meting_id = musicPlayerConfig.meting?.id ?? "14164869977";
		const meting_server = musicPlayerConfig.meting?.server ?? "netease";
		const meting_type = musicPlayerConfig.meting?.type ?? "playlist";

...
export function transformMemosToTalking(
	memos: Memo[],
	baseUrl: string,
): TalkingItem[] {
	return memos
...
function getImageLayoutClass(count: number): string {
	if (count === 1) {
		return "talking-images-single";
	}
	if (count === 2) {
		return "talking-images-double";
	}
	if (count === 3) {
		return "talking-images-triple";
	}
	return "talking-images-grid";
}
...
function renderMomentCard(
	...
	if (moment.images && moment.images.length > 0) {
		...
		const imgs = moment.images
			...
			.join("");
		imagesHtml = `<div class="talking-images grid gap-2 mb-3 ${layoutClass}">${imgs}</div>`;
	}
